...
const priceBadge = 'inline-flex px-3 py-1 rounded-xl bg-[#FFF3CC] border-2 border-[#F9C74F] font-accent text-2xl text-[#8B6914]';
                  ...
                  <tbody>
                    {flowers.map((f, idx) => (
                      <tr key={f.id} className={`border-b border-dashed border-astraea-pink/20 hover:bg-astraea-blush/20 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-astraea-cream'}`}>
                        ...
                        <td className="p-4"><span className={priceBadge}>₱{f.price_per_stem}</span></td>
                        <td className="p-4"><ColorDots colors={flowerColors.filter(c => c.flower_id === f.id)} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <tbody>
                  {fillers.map((f, idx) => (
                    <tr key={f.id} className={`border-b border-dashed border-astraea-pink/20 hover:bg-astraea-blush/20 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-astraea-cream'}`}>
                      ...
                      <td className="p-4"><span className={priceBadge}>₱{f.price}</span></td>
                      <td className="p-4 text-right">
                        <span className={`kawaii-badge ${f.is_available && Number(f.stock || 0) > 0 ? 'bg-[#D5F0E8] border-[#A8DFC9] text-[#2D7A5F]' : 'bg-[#FDDDE6] border-[#F9A8C9] text-[#C4658A]'}`}>
                          {f.is_available && Number(f.stock || 0) > 0 ? 'Available' : 'Out of Stock'}
                        </span>
                      </td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {wrappers.map((w, idx) => (
                    <tr key={w.id} className={`border-b border-dashed border-astraea-pink/20 hover:bg-astraea-blush/20 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-astraea-cream'}`}>
                      ...
                      <td className="p-4"><span className={priceBadge}>₱{w.price}</span></td>
                      <td className="p-4"><ColorDots colors={wrapperColors.filter(c => c.wrapper_id === w.id)} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section>
              ...
              <div className="scrapbook-card washi-strip bg-[#FFFDFE] space-y-4">
                {addons.map(addon => (
                  <div key={addon.id} className="flex justify-between items-center border-b border-dashed border-astraea-pink/20 pb-2">
                    <span className="font-medium">{addon.name}</span>
                    <span className={priceBadge}>₱{addon.price}</span>
                  </div>
                ))}
            ...
            <section>
              ...
              <div className="scrapbook-card washi-strip bg-[#FFFDFE] overflow-hidden">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {sizes.map((size, idx) => (
                      <tr key={size.id} className={`${idx < sizes.length - 1 ? 'border-b border-dashed border-astraea-pink/20' : ''} hover:bg-astraea-blush/20`}>
                        <td className="p-4 font-medium">{size.name}</td>
                        <td className="p-4"><span className={priceBadge}>₱{size.base_price}</span></td>
                        <td className="p-4 text-right text-astraea-darkgray/70">{size.stems}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
    ...
    {colors.map(c => (
      <div key={c.id} className="relative group">
        <div className={`w-8 h-8 rounded-full border-2 ${!c.is_available ? 'border-[#C4658A] opacity-50' : 'border-white'} shadow-[2px_2px_0px_#F9A8C9]`} style={{ backgroundColor: c.hex_code }}>
          {!c.is_available && <div className="absolute inset-0 flex items-center justify-center"><div className="w-full h-0.5 bg-[#C4658A] rotate-45"></div></div>}
        </div>
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block z-10 w-max rounded-xl bg-[#FDDDE6] text-[#C4658A] text-xs px-2 py-1 font-accent border border-[#F9A8C9] shadow-[2px_2px_0px_#F9A8C9]">
          {c.color_name}
        </div>
      </div>
    ))}
